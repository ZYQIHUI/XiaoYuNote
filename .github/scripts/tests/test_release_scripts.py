    ...
    metadata_dir.joinpath("appcast.xml").write_text(
        "\n".join(
            [
                ...
                (
                    f'      <enclosure url="{release_base}/XiaoYuNote-{version}-macos-arm64.dmg" '
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                ...
                [
                    "version=1.2.3",
                    "tag=1.2.3",
                    "release_name=XiaoYuNote v1.2.3：稳定发布",
                    "macos_asset=XiaoYuNote-1.2.3-macos-arm64.dmg",
                    "windows_asset=XiaoYuNote-1.2.3-windows-x64-setup.exe",
                ],
            )
    ...
    def test_update_metadata_round_trip(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_result = run_script(
                ...
                "--windows-asset",
                "XiaoYuNote-1.2.3-windows-x64-setup.exe",
                "--notes",
                str(notes),
                "--output-dir",
                str(output_dir),
                ...
                json.dumps(
                    {
                        "version": "1.2.3",
                        "change_time": "2026年6月29日 13:30:00",
                        "download_url": f"{release_base}/XiaoYuNote-1.2.3-macos-arm64.dmg",
                    }
                ),
                encoding="utf-8",
            )
                ...
                json.dumps(
                    {
                        "version": "1.2.3",
                        "change_time": "2026年6月29日 13:30:00",
                        "download_url": f"{release_base}/XiaoYuNote-1.2.3-windows-x64-setup.exe",
                    }
                ),
                encoding="utf-8",
            )
            ...
            metadata_dir.joinpath("appcast.xml").write_text(
                "\n".join(
                    [
                        ...
                        "    <item>",
                        (
                            f'      <enclosure url="{release_base}/XiaoYuNote-1.2.3-windows-x64-setup.exe" '
